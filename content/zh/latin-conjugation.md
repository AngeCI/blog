---
date: "2026-05-28"
type: "post"
title: "拉丁語動詞變位"
description: ""
translationsKey: "latin-conjugation"
categories:
  - "Linguistics 語言學"
  - "Notes 筆記"
tags:
  - "Latin 拉丁語"
---

# 序言
拉丁語的動詞變位由 4-5 個相互交織的維度共同決定：
- 變位法（conjugation class）
- 人稱及人數（person & number）：三種人稱各自有單數和複數，合共有六種組合。
- 時–體（tense-aspect）
- 語態（voice）：主動與被動（active & passive）。
- 語氣（mood）：直陳、虛擬與祈使（indicative, subjunctive & imperative）。

# 動詞分類
拉丁語的動詞詞目通常有四個條目，展示了拉丁語動詞的四種詞幹。

拉丁語的動詞傳統上分為四種變位法，可從動詞的第二條目的結尾推斷：

| 變位法 | 主動 | 被動 |
| -- | -- | -- |
| 1st (ā-stem) | -āre | -ārī |
| 2nd (ē-stem) | -ēre | -ērī |
| 3rd (ĕ/ĭ-stem) | -ere | -ī |
| 4th (ī-stem) | -īre | -īrī |

和[名詞](/blog/zh/latin-nouns-declension/)一樣，動詞的第三變位法有兩個亞種，它們在第一條目的表現有點不一樣。

如果一個拉丁語動詞的不定式結尾與上表列出的不符，那它就是無法被歸入任一變格法的不規則動詞。

拉丁語的動詞變位法實際上是兩種系統的交織。大多數比較能產的第一和第二變位法動詞，完成詞幹是從現在詞幹推出來的。而第三變格法動詞的完成詞幹，則通常來自另一較古老的層次，詞幹構成方式多種多樣。下表顯示各種變位法的各種時態第一人稱單數主動形式：

|  | 1st (ā-stem) | 2nd (ē-stem) | 3rd (ĕ-stem) | 3rd (ĭ-stem) | 4th (ī-stem) |
| -- | -- | -- | -- | -- | -- |
| 現在時 | -ō | -eō | -ō | -iō | -iō |
| 不定式 | -āre | -ēre | -ere | -ere | -īre |
| 未完成時 | -ābam | -ēbam | -ēbam  | -ēbam  | -iēbam |
| 將來時 | -ābō | -ēbō | -am  | -iam  | -iam |
| 完成時 | -āvī | -uī | -ī | -ī | -īvī |
| 過去分詞 | -ātum | -itum | -tum | -tum | -ītum |

請注意不是所有動詞的完成時詞幹和過去分詞詞幹都可以直接從現在詞幹（字典的第二條目）推出，雖然說「大多數」第一、第二和第四變位法的動詞都可以這麼推。

## 鼻音中綴現在時
這類動詞的現在詞幹有一個鼻音中綴（一般拼成 n，在 b 或 p 前拼成 m），而完成詞幹則沒有這個鼻音中綴，並且詞根元音通常會變成長元音。

| 假想詞根 | 現在時 | 完成時 | 過去分詞 |
| -- | -- | -- | -- |
| √liqu- | linquō | līquī | lictum |
| √vic- | vincō | vīcī | victum |
| √rup- | rumpō | rūpī | ruptum |
| √fud- | fundō | fūdī | \*fud-tum &gt; \*fussum &gt; !fūsum |
| √iug- | iungō | \*iūxī &gt; !iūnxī | \*iuctum &gt; !iūnctum |
| √pos- | \*posnō &gt; pōnō | posuī | postum &gt; !positum |

## 重疊現在時
| 假想詞根 | 現在時 | 完成時 | 過去分詞 |
| -- | -- | -- | -- |
| √gen- | gignō | genuī | genitum |

# s- 完成時
這類動詞的完成詞幹有一個 -s- 中綴。這個中綴會與前面緊鄰的輔音產生各種變化。

| 假想詞根 | 現在時 | 完成時 | 過去分詞 |
| -- | -- | -- | -- |
| √duc- | dūcō | dūxī | ductum |
| √scrīb- | scrībō | scrīpsī | scrīptum |
| √coqu- | coquō | coxī | coctum |
| √sūm- | sūmō | sūmpsī | sūmptum |
| √ges- | \*gesō &gt; gerō | gessī | gestum |
| √spec- | speciō | spexī | spectum |

## 重疊完成時
這類動詞的完成詞幹有一個重疊音節。

| 假想詞根 | 現在時 | 完成時 | 過去分詞 |
| -- | -- | -- | -- |
| √st- | stō | stetī | statum |
| √st- | sistō | stitī | statum |
| √d- | dō | dedī | datum |
| √curr- | currō | cucurrī | cursum |
| √cad- | cadō | cecidī | \*cad-tum &gt; \*cassum &gt; !cāsum |
| √caed- | caedō | cecīdī | caesum |
| √can- | canō | cecinī | cantum |
| √parc- | parcō | pepercī | parsum |
| √scid- | scindō | scicidī | scissum |
| √pug- | pungō | pupugī | \*puctum &gt; !pūnctum |
| √tag- | tangō | tetigī | \*tactum &gt; !tāctum |

## -esc- 動詞

# 第一、第二條目
|  | 1st (ā-stem) | 2nd (ē-stem) | 3rd (ĕ-stem) | 3rd (ĭ-stem) | 4th (ī-stem) |
| -- | -- | -- | -- | -- | -- |
| 現在時 | -ō | -eō | -ō | -iō | -iō |
| 不定式 | -āre | -ēre | -ere | -ere | -īre |
| 未完成時 | -ābam | -ēbam | -ēbam  | -ēbam  | -iēbam |
| 將來時 | -bō | -bō | -am | -iam | -iam |
| 現在時被動 | -or | -eō | -ō | -iō | -iō |
| 未完成時被動 | -ābar | -ēbar | -ēbar  | -ēbar  | -iēbar |
| 將來時被動 | -bō | -bō | -ar | -iō | -iō |

|  | egō | tū | is, ea, id | nōs | vōs | eī, eae, ea |
| -- | -- | -- | -- | -- | -- | -- |
| 現在時 | -ō | -s | -t | -mus | -tis | -nt |
| 未完成時 | -bam | -bās | -bat | -bāmus | -bātis | -bant |
| 將來時（1, 2） | -bō | -bis | -bit | -bimus | -bitis | -bunt |
| 將來時（3, 4） | -am | -ēs | -et | -ēmus | -ētis | -ent |
| 現在時被動 | -or | -ris | -tur | -mur | -minī | -ntur |
| 未完成時被動 | -bar | -bāris | -bātur | -bāmur | -bāminī | -bantur |
| 將來時被動（1, 2） | -bor | -beris | -bitur | -bimur | -biminī | -buntur |
| 將來時被動（3, 4） | -ar | -ēris | -ētur | -ēmur | -ēminī | -entur |

# 第三條目
|  | egō | tū | is, ea, id | nōs | vōs | eī, eae, ea |
| -- | -- | -- | -- | -- | -- | -- |
| 完成時 | -ī | -istī | -it | -imus | -istis | -ērunt |
| 過去完成時 | -eram | -erās | -erat | -erāmis | -erātis | -erant |
| 將來完成時 | -er | -eris | -erit | -erimus | -eritis | -erint |
| 完成時 | -erim | -erīs | -erit | -erīmus | -erītis | -erint |
| 過去完成時 | -issem | -issēs | -isset | -issēmus | -issētis | -issent |
