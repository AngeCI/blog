---
date: "2026-09-15T16:00:00+00:00"
draft: true
type: "post"
title: "BlogBlog 同樂會：語言的習得、傳承與推廣（2026 年 10 月）"
description: ""
translationsKey: "blogblog-party-languages"
categories:
  - "BlogBlog 同樂會"
tags:
  - "Language Policy 語言政策"
---

> [!WARNING] Notice
> This post is a draft translation from [the Chinese version](/blog/zh/blogblog-party-languages/) which have not yet been thoroughly proofread.

> [!IMPORTANT]
> 9 月的 [BlogBlog 同樂會：晚上不睡覺](https://jasonjlai.net/zh/3pwriting/stay-up.html)現在還在火熱進行中，9 月底之前都能投稿喔！

本人在 2026 年 3 月就已經有主持 BlogBlog 同樂會的想法，當時最初是排在 [ikuka](https://ikukaroom.com) 後面，只是由於本人在 9 月份的現實工作稍忙，而把主持時間後延了一個月到了 10 月。

本網站存在的其中一個理由，是保存各種易於閱讀的[筆記](/blog/zh/categories/notes-%E7%AD%86%E8%A8%98/)，包括但不限於各種語言的學習與推廣。目前我這個網站好像還蠻多[語言學](/blog/zh/categories/linguistics-%E8%AA%9E%E8%A8%80%E5%AD%B8/)文章的，不過其實我本身的專業可以說是和語言學完全風馬牛不相及、八桿子打不着關係。

不知道大家有沒有留意到，這個網站實際上是[多語](/blog/zh/multilingual-blog/)的，雖然很多時候我往往都是先發佈其中一種語言版本，以後有時間再慢慢翻譯其他的語言版本。

# 主題：「語言的習得、傳承與推廣」
2026 年 10 月的主題是：「**語言的習得、傳承與推廣**」！

面對着本土語言衰落的境況[^1]，各位格友們對此有何感想、意見或建議？事實上下列的「[本土語言的發展和保存](#-本土語言的發展和保存)」就是我最一開始所擬定的主題。

> **題外話**：說起本土語言，目前網路上似乎不是很能找到適合零基礎的外國人學習的台語／閩南語教材，與之相關的自然語言處理系統也還是在發展階段，也許這是個巨大的市場缺口。也許將來會考慮嘗試弄一個自研教學平台。


# 選題靈感
以下有一些可供參考的選題靈感。

## 💬 本土語言的發展和保存
這是我一開始擬定的題目。後來和一些其他人討論過後覺得原本的題目有點太窄，把題目拓寬成了現在的樣子。

- 👨‍👩‍👧‍👦 **家庭及社會層面的語言政策**：大家「如果」打算組織家庭、生兒育女的話，打算對家庭、子女和社區實行怎樣的語言政策？
- 🗳️ **政府層面的語言政策**：假設你有能力從政府層面制定政策，你會給你的國民制定怎樣的語言政策？
- 💬 **本土語言的用字考究、用語、俗語等的介紹？** 真要寫的話我應該可以弄出來一整個文章分類，只得我目前有點懶得動筆。
- 👴 **尋根**：在語言斷層出現之後，重拾祖輩語言的心路歷程？
- 🏴🏫 **極端的語言復興主義**：想像一下一個三戰之後的世界，假如你是這個時代的小孩，被一群政治狂熱份子送進「本土語言學校」強迫接受本土語言教育（甚至所有科目皆以本土語言講授），你會否感到痛苦、追不上學習進度，然後開始討厭上學？甚至因為國語被視為殖民者的語言，讓本土語言說的不好的成年人也被強迫接受「再教育」？
- 💻 **低資源語言的自然語言處理**：這是一個比較偏向技術層面的面向。一些本土語言的自然語言處理技術發展較晚，各位對此有無建議？可能的實作方向或者產品都可以。各位如何投資本土語言／低資源語言的語音合成與辨識、斷詞、標音、翻譯、TTS/RVC 音源等技術？
- 🩸🔫🙊😱 還是說，比起本土語言的保育，你更樂於[以強勢語言打壓及侮辱本土語言](/blog/zh/alex-hsu-suppresses-local-languages/)？

## 🌐 評論各國政府的語言政策
**對各國政府語言政策的評論。** 可以是歷史上的或者是現在進行式的。

- 📖 目前所有的華文地區皆要求在義務教育階段研習文言文。聽說日本的義務教育也會修讀文言文內容。[學習文言文在現代社會有實際用處嗎？](/blog/zh/studying-classical-chinese/)
- 🖋️ [漢字書法](/blog/zh/chinese-calligraphy/)是反人體工學的嗎？應否將漢字書法納入到必修的義務教育當中？

## 📚 學習語言的經驗
- 💬 你是否曾經有被要求學習多種語言，然後發現學習進度根本追不上的經驗？
- 💬 你最喜歡的語言、學過的語言、最想學的語言、學了很久還是學不好的語言？
- 🧑‍🏫 遇到過的語言老師、交過的不同語言的朋友
- 📚 自己私藏的語言學習法

## ❓ 其他
- 🗣️ 因為語言／口音不同而發生的趣事、culture shock 或者歧視經驗
- 🐱 跟寵物溝通的語言、使用不同語言（甚至程式語言）的經驗
- 💡 各種有關語言的冷知識，似乎和[幾個月前的同樂會主題](https://shuaixin.cc/Fun-Fact/)有點兒重疊（但我不會禁止在往期同樂會投稿過的舊文重複投稿！）。

以上提示僅供參考，參與者可以自由發揮，只要內容和「語言的習得、傳承與推廣」有關即可。

# 參加方式
1. 在自己的部落格上寫一篇跟「語言的習得、傳承與推廣」有關的文章（可以的話，複製下方的文字放在文章開頭，但不強制）
    > [!NOTE] 在投稿文章開頭放這段文字（非強制）
    >
    > 這是我的「[BlogBlog 同樂會 - 2026 年 10 月](https://blogblog.club/party)」的投稿文章。本月主題是「[語言的習得、傳承與推廣](https://angeci.github.io/blog/zh/blogblog-party-languages/)」，由 [AngeCI](https://angeci.github.io/blog/zh/) 主持。如果你有自己的部落格，歡迎一起來參加！

    如果你用 markdown 語法寫作，可以直接複製下行：
    ```md
    > 這是我的「[BlogBlog 同樂會 - 2026 年 10 月](https://blogblog.club/party)」的投稿文章。本月主題是「[語言的習得、傳承與推廣](https://angeci.github.io/blog/zh/blogblog-party-languages/)」，由 [AngeCI](https://angeci.github.io/blog/zh/) 主持。如果你有自己的部落格，歡迎一起來參加！
    ```
2. 把文章連結（不是全文喔）寄到我的信箱 angeci (at) ltgc.cc [^2]
3. 信件主旨可寫「BlogBlog 同樂會投稿：語言的習得、傳承與推廣」

    信件內文參考格式：
    ```md
    [文章名稱](https://blog-post-url) - 投稿人稱呼

    投稿附言（可選）
    ```
    <span class="chide">如果您投稿時沒有向本人清晰指明投稿人稱呼的話，我會嘗試從往期投稿名單尋找配對。要是配對不了的話我就可能只能從電郵標頭或部落格標題隨便亂抓了。</span>
4. 記得在截稿日（2026-10-31）[^3]前寄出！（即日起就可以開始投稿）

# Q & A
- **Q1:** 沒有自己的部落格也能參加嗎？

    **A1:** 如果是「自己」的部落格更好。但文章如果是在 Substack、Medium、方格子也 OK。不接受社群媒體的投稿（如 Facebook、Instagram、Threads、X）。請確保投稿文章在截稿時公開可見。如果我在中途或截稿時發現文章無法公開檢視的話，我會嘗試通知有關的投稿者，一段時間之內沒有恢復／更新的話我會把失效／無法檢視的連結直接移除。
- **Q2:** 一定要寫新文章嗎？可以拿之前寫的舊文章參加嗎？

    **A2:** 可以拿過去的舊文章參加唷！如果你部落格裡剛好有符合主題的文章，可以直接投稿，不用因為文章是去年寫的就不好意思拿出來。只要符合「**語言的習得、傳承與推廣**」這個主題的文章都可以參加。（但為同樂會專門寫一篇新的文章，會更有參與感喔！）
- **Q3:** 投稿有篇數上限嗎？

    **A3:** **原則上不設限**[^8]，這可能是歷屆同樂會以來最佛心的一次了 ~~，但也請別刻意灌水滋擾我拜託🤣~~。
- **Q4:** 投稿語言有限制嗎？

     **A4:** 原則上無限制，不過 BlogBlog 同樂會的投稿活動一般以繁體中文為主。視乎本人對不同語種的熟悉程度，我有可能需要依賴機器翻譯服務來理解閣下的外語投稿。不過本人台語程度較弱[^4]，若閣下投稿「只」有台語的文章而未有提供翻譯（翻譯要公開或者私下提供都可以），我將會需要依賴機器翻譯服務輔助理解。
- **Q5:** 投稿後多久會收到回覆？[^5]

    **A5:** 儘可能在一至兩天內回覆吧[^6]。如果我剛好比較沒空或是您在臨近截稿期限時投稿的話，我可能會先出一封回信確認收件，之後才另外補上針對文章內容的回應（或者直接在回顧文中回覆）。您亦可以隨時造訪本頁的投稿文章列表檢查本人是否已確認收件。**但我目前用的是免費郵箱服務，不保證回顧文發佈時會給大家統一發 newsletter。**

# 已經投稿的作者清單（持續更新）
活動期間，這裡會持續更新已投稿的文章連結，歡迎大家互相閱讀、交流、訂閱！最終整理與回顧文章會在 11 月初發布。

以下是目前參與文章的連結列表（按投稿順序）：

1. [淺談數個漢語方言現象](/blog/zh/chinese-dialectal-phenomena/) - AngeCI
1. [論學習文言的必要性](/blog/zh/studying-classical-chinese/) - AngeCI
1. [論漢字書法教育](/blog/zh/chinese-calligraphy/) - AngeCI
1. [多語部落格](https://angeci.github.io/blog/zh/multilingual-blog/) - AngeCI
1. [統一譯名](https://angeci.github.io/blog/zh/unifying-names/) - AngeCI
1. [Alex Hsu 打壓本土語言](/blog/zh/alex-hsu-suppresses-local-languages/) - AngeCI

[^1]: 別以為是只有台灣是這樣，其實大陸甚至香港都有。
[^2]: 請注意，本人用於發送回郵的電郵地址將與接收郵件的地址不同。各位要再回覆郵件的話，可以在兩個郵件地址之間任擇其一。
[^3]: 由於「[BlogBlog 同樂會主持人手冊](https://blogblog.club/docs/guides/blogblog-party-host-guide)」裏面沒有指明截稿日所依據的時區，我可能會從寬認定。目前地球上正在使用的最晚的時區是 UTC-12，比台灣時間晚了 20 小時，我個人的截稿時間可能以這個為準，稱作 [AoE（anywhere on earth）時間](https://zh.wikipedia.org/wiki/AoE%E6%97%B6%E9%97%B4)（順帶一提最早的時區是 UTC+14）。如果閣下在截稿時間過後、回顧文發佈前投稿，我有可能會接受閣下的逾期投稿，但不保證我會趕及在回顧文發佈前將閣下的逾期投稿整理進去。
[^4]: 本人是香港人，我家也沒有祖籍是閩南或者潮汕的親戚，因此閩南語其實不算是我的 heritage language[^7]，我也不會有在童年自然習得閩南語的環境。我僅有的閩南語能力都是在我長大之後基於對外語的興趣才開始學習的。<!--另外網絡上的閩南語的正字法狀況一團糟，似乎讓 AI 看了都無所適從 ~~，所以也別指望我會嘗試透過機器翻譯服務理解台文了……~~-->
[^5]: <span class="hovers-blur">對，我就是個比較急性子的人。我比較喜歡可以掌控一切的感覺，而不太喜歡無了期等待的[不確定性](https://wiwi.blog/blog/uncertainty-is-okay/)。</span>
[^6]: <span class="hovers-blur">如果我長時間未有回覆閣下的郵件，而我又有將閣下的投稿更新到投稿列表中，那你大概率是幹了什麼齷齪事讓我選擇刻意延遲回覆了。同理，如果你提交原文是繁體，但本頁列出的文章標題被強制轉換成簡體的話，那麼你可以好好思考一下你做了什麼讓我決定這麼做。</span>
[^7]: 其實香港人祖籍在潮汕或閩南的比例其實不低（但不至於壓倒性優勢），但我自己剛好不是。實際上香港中老年人能講這些 heritage langauge 的比例已經不多，年輕一代連聽懂都成問題。有關香港的語言發展史，我日後也許會再出一篇專題文章。
[^8]: 但是不排除會因為本人的突發事件而收緊。
